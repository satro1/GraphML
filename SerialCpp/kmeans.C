 ...
 * */
vector<vector<double>> getRandomCentroids(vector<vector<double>> nodes,
                                          int dim) {
...
int getIndexOfClosestCentroid(vector<vector<double>> centroids,
                              vector<double> element, int dim) {
...
int main() {
  ...
  vector<vector<vector<double>>> clusters(
      NUM_CLUSTERS,
      vector<vector<double>>(elems.size(), vector<double>(DIM, 0)));
  vector<int> sizes(NUM_CLUSTERS, 0);
  map<vector<double>, int> elementToClusterIndex; // map(vector<double> => int
                                                  // index of cluster in vector)

  // place the elements into their initial clusters
  for (int i = 0; i < elems.size(); i++) {
    // find which centroid its closest to
    int indexOfClosestCentroid =
        getIndexOfClosestCentroid(centroids, elems[i], DIM);
    clusters[indexOfClosestCentroid][sizes[indexOfClosestCentroid]++] =
        elems[i];
    elementToClusterIndex.insert(
        pair<vector<double>, int>(elems[i], indexOfClosestCentroid));

    cout << "Element " << i << " was found to be closest to index "
         << indexOfClosestCentroid;
    cout << "\nELEMENT: ";
    for (int j = 0; j < DIM; j++) {
      cout << elems[i][j] << ",";
    }
    ...
    cout << "with a distance of "
         << distanceBetweenVectors(centroids[indexOfClosestCentroid], elems[i],
                                   DIM)
         << "\n";
  }
  ...
  while (!convergence) {
    ...
    for (int i = 0; i < elems.size(); i++) {
      ...
      // elementToClusterIndex.insert(pair<vector<double>, int>(elems[i],
      // newIndex));
  ...
  for (int i = 0; i < elems.size(); i++) {
    // find min distance
    if (getIndexOfClosestCentroid(centroids, elems[i], DIM) !=
        elementToClusterIndex[elems[i]]) {
